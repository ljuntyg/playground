...
Vector3 objl::math::ProjV3(const Vector3 a, const Vector3 b) {
...
Vector3 objl::algorithm::GenTriNormal(Vector3 t1, Vector3 t2, Vector3 t3) {
    Vector3 u = t2 - t1;
    Vector3 v = t3 - t1;
...
inline void objl::algorithm::split(const std::string &in, std::vector<std::string> &out, std::string token) {
    out.clear();
...
template <class T>
inline const T & objl::algorithm::getElement(const std::vector<T> &elements, std::string &index) {
